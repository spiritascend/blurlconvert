...
func DownloadAndConcatSegment(mastertrack string, masterplaylisturl string, initmp4 string, adaptation string, maxidx int, segmentindex int) error {
	...
	if resp.StatusCode == 404 && segmentindex == maxidx {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("bad status when downloading segment (ctx_url: %s): %s", url, resp.Status)
	}
...
func HandleDownloadTrack(id string, numberofsegments float64, baseurl string, initmp4 string, adaptation string, key string) error {
	...
	for idx := 0; idx < segmentCount; idx++ {
		err := DownloadAndConcatSegment(initmp4, baseurl, initmp4, adaptation, int(numberofsegments), idx+1)
...
func HandleDownloadTrackV2(mediatype string, id string, numberofsegments float64, baseurl string, initmp4 string, adaptation string, key string) error {
	segmentCount := int(numberofsegments)
